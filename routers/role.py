...
@router.get("/search")
def role_search(env: str = Query("HCM"), q: str = Query(""), limit: int = Query(100)):
...
@router.get("/{rolename}")
def role_detail(rolename: str, env: str = Query("HCM")):
...
def role_members(rolename: str, env: str = Query("HCM")):
...
def role_permissionlists(rolename: str, env: str = Query("HCM")):
